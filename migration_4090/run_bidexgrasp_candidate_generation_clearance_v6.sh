...
log="$repo/migration_4090/logs/bidexgrasp_bleach_candidate_generation_clearance_v6.log"
...
"$conda_root/bin/conda" run --no-capture-output -n tro \
  ...
  --hand-variants 5 \
  --minimum-pair-clearance-mm 20 \
  --maximum-pair-clearance-mm 30 \
  ...
  --friction 1 \
  --seed 20260806 \
  --output migration_4090/derived/source_vis_bleach_bidexgrasp_candidates_clearance_v6.pt \
  --audit-json migration_4090/results/bidexgrasp_bleach_candidate_audit_clearance_v6.json
