#!/usr/bin/env bash
set -uo pipefail
GPU="${1:?gpu}" FRICTION="${2:?friction}" SCALE="${3:?scale}" TAG="${4:?tag}"
...
OUT=migration_4090/results/xhand_targeted_pose_search_v2/param_probe
mkdir -p "$OUT"
"$ISAAC" migration_4090/validate_xhand_fullbody_isaac.py \
  ...
  --density 100 --friction "$FRICTION" --hand-close-scale "$SCALE" --closure-mode teleport \
