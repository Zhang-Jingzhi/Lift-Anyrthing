...
OUT=migration_4090/results/xhand_targeted_pose_search_v1/physics_screen
mkdir -p "$OUT"
for ((i=START; i<END; i++)); do
  report="$OUT/${METHOD}_$(printf '%03d' "$i").json"
  if [ -s "$report" ]; then echo "SKIP $report"; continue; fi
  echo "START gpu=$GPU method=$METHOD index=$i"
  ...
  echo "DONE gpu=$GPU method=$METHOD index=$i"
done
