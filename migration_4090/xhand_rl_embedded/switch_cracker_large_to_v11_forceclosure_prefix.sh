...
output="$root/runs/cracker_large/training_retry_016"
log="$root/logs/cracker_large/shaped_v11_forceclosure_prefix_switch.log"
...
if not any(Path(row["checkpoint"]).resolve() == seed and row.get("saved_before_ppo_update") is True and int(row.get("max_level", 0)) >= 3 and row.get("checkpoint_sha256") == sha256_file(seed) for row in rows):
    raise SystemExit("no hash-verified level-3 prefix evidence")
if resume_checkpoint_iteration(seed) != 1826:
    raise SystemExit("unexpected source iteration")
PY

screen -S xhand_formal_embedded_reconcile_target100 -X quit >/dev/null 2>&1 || true
screen -S xhand_formal_v10_prefix_cracker_large -X quit >/dev/null 2>&1 || true
echo "$(date --iso-8601=seconds) stopped cracker_large retry_015; cooldown_s=90" >> "$log"
sleep 90
...
screen -L -Logfile "$root/logs/cracker_large/shaped_v11_forceclosure_prefix_training_screen.log" \
  -dmS xhand_formal_v11_prefix_cracker_large \
  env PYTHONPATH="$repo" PYTHONUNBUFFERED=1 "$python" -m migration_4090.xhand_rl_embedded.train \
    ...
    --num-envs 128 --max-iterations 5826 --seed 3886 --device cuda:2 --headless \
    --kit-portable-root /tmp/xhand_rl_embedded_kit/v11_prefix/cracker_large \
    ...
    --stable-lift-reward-weight 24.0 --force-closure-reward-weight 36.0 \
...
sleep 20
if ! ps -ww -eo args= | rg -q 'migration_4090\.xhand_rl_embedded\.train.*--object cracker_large.*training_retry_016'; then
  echo "$(date --iso-8601=seconds) failed to verify cracker_large retry_016" >> "$log"; exit 4
fi
echo "$(date --iso-8601=seconds) launched cracker_large training_retry_016 from level-3 prefix" >> "$log"
