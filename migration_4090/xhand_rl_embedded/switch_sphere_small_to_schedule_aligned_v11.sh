...
output="$root/runs/sphere_small/training_retry_020"
log="$root/logs/sphere_small/shaped_v11_schedule_aligned_switch.log"
...
screen -S xhand_formal_v10_prefix_sphere_small -X quit >/dev/null 2>&1 || true
echo "$(date --iso-8601=seconds) stopped sphere_small retry_019; cooldown_s=90" >> "$log"
sleep 90
...
screen -L -Logfile "$root/logs/sphere_small/shaped_v11_schedule_aligned_training_screen.log" \
  -dmS xhand_formal_v11_schedule_sphere_small \
  env PYTHONPATH="$repo" PYTHONUNBUFFERED=1 "$python" -m migration_4090.xhand_rl_embedded.train \
    ...
    --num-envs 128 --max-iterations 4300 --seed 3685 --device cuda:1 --headless \
    --kit-portable-root /tmp/xhand_rl_embedded_kit/v11_schedule/sphere_small \
    ...
    --residual-activation-phase 1 --instantaneous-residual-weight 0.0 \
...
sleep 20
if ! ps -ww -eo args= | rg -q 'migration_4090\.xhand_rl_embedded\.train.*--object sphere_small.*training_retry_020'; then
  echo "$(date --iso-8601=seconds) failed to verify sphere_small retry_020" >> "$log"; exit 4
fi
echo "$(date --iso-8601=seconds) launched sphere_small training_retry_020 with schedule-aligned formal phases" >> "$log"
