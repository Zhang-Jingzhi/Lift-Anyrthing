...
source="$root/runs/pyramid/training_retry_021"
seed="$source/hard_prefix_preupdate_iter_722_level_2.pt"
output="$root/runs/pyramid/training_retry_022"
log="$root/logs/pyramid/shaped_v10_clearance_prefix_switch.log"
...
if resume_checkpoint_iteration(seed) != 722:
    raise SystemExit("unexpected source iteration")
PY

screen -S xhand_formal_embedded_reconcile_target100 -X quit >/dev/null 2>&1 || true
screen -S xhand_formal_v9_prefix_pyramid_clearance -X quit >/dev/null 2>&1 || true
echo "$(date --iso-8601=seconds) stopped pyramid retry_021; cooldown_s=90" >> "$log"
sleep 90
...
screen -L -Logfile "$root/logs/pyramid/shaped_v10_clearance_prefix_training_screen.log" \
  -dmS xhand_formal_v10_prefix_pyramid \
  env PYTHONPATH="$repo" PYTHONUNBUFFERED=1 "$python" -m migration_4090.xhand_rl_embedded.train \
    ...
    --num-envs 128 --max-iterations 4722 --seed 3588 --device cuda:5 --headless \
    --kit-portable-root /tmp/xhand_rl_embedded_kit/v10_prefix/pyramid \
...
sleep 20
if ! ps -ww -eo args= | rg -q 'migration_4090\.xhand_rl_embedded\.train.*--object pyramid.*training_retry_022'; then
  echo "$(date --iso-8601=seconds) failed to verify pyramid retry_022" >> "$log"; exit 4
fi
echo "$(date --iso-8601=seconds) launched pyramid training_retry_022" >> "$log"
