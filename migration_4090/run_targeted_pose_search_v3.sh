...
ROOT=migration_4090/results/xhand_targeted_pose_search_v3
mkdir -p "$ROOT/raw"
run_one() {
...
active=0
for x in 0.011 0.012 0.013; do
  for z in 0.047 0.048 0.049; do
    for side in 0.684 0.685 0.686; do
      run_one "$x" "$z" "$side" &
      active=$((active+1))
...
"$TRO" - <<'PY'
import glob,torch
ps=sorted(glob.glob('migration_4090/results/xhand_targeted_pose_search_v3/raw/baseline_*/*.pt'))
rows=[]
for p in ps: rows.extend(torch.load(p,map_location='cpu')['samples'])
out='migration_4090/results/xhand_targeted_pose_search_v3/baseline.pt'
