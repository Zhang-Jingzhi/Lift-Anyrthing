...
OUT=migration_4090/results/xhand_targeted_pose_search_v2/physics_full
mkdir -p "$OUT"
"$ISAAC" migration_4090/validate_xhand_fullbody_isaac.py \
  --dataset "$DATA" --sample-index "$INDEX" --output "$OUT/baseline_$(printf '%03d' "$INDEX").json" --gpu "$GPU" \
