...
OUT=migration_4090/results/xhand_targeted_pose_search_v3/canonical_full_probe
mkdir -p "$OUT"
"$ISAAC" migration_4090/validate_xhand_fullbody_isaac.py \
  --dataset migration_4090/results/xhand_targeted_pose_search_v3/baseline_canonical.pt --sample-index "$INDEX" \
  --output "$OUT/baseline_$(printf '%03d' "$INDEX").json" --gpu "$GPU" --modes both \
  --density 100 --friction 2.0 --hand-close-scale 1.05 --closure-mode teleport \
