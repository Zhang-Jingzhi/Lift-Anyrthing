#!/usr/bin/env bash
set -uo pipefail
GPU="${1:?gpu}" INDEX="${2:?index}" TAG="${3:?tag}" MODES="${4:-both}"
...
OUT=migration_4090/results/xhand_targeted_pose_search_v3/verified_probes
mkdir -p "$OUT"
"$ISAAC" migration_4090/validate_xhand_fullbody_isaac.py \
  ...
  --output "$OUT/${TAG}.json" --gpu "$GPU" --modes "$MODES" \
