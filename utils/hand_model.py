...
class HandModel:
    ...
    def get_transformed_links_pc(self, q=None, links_pc=None):
        ...
        for link_index, (link_name, link_pc) in enumerate(links_pc.items()):
            ...
            all_pc_se3[link_name] = link_pc_se3
            all_link_se3.append(se3)

        all_link_se3 = torch.stack(all_link_se3, dim=0)
        return all_pc_se3, all_link_se3

    def get_transformed_links_pc_from_se3(self, se3, links_pc=None):
        
        if links_pc is None:
            links_pc = self.links_pc
        all_pc_se3 = []
        for link_id, (link_name, link_pc) in enumerate(links_pc.items()):
            homo = torch.ones(link_pc.shape[0], 1, device=link_pc.device)
            link_pc_homo = torch.cat([link_pc, homo], dim=1)

            link_se3 = se3[:, link_id]
            link_pc_homo = link_pc_homo.unsqueeze(0).expand(link_se3.shape[0], -1, -1)
            link_pc_se3 = torch.bmm(link_pc_homo, link_se3.transpose(1, 2))
            all_pc_se3.append(link_pc_se3[:, :, :3])
        return torch.cat(all_pc_se3, dim=1)
